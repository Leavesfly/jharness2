...
public class McpManager implements AutoCloseable {
    ...
    /**
     * 添加本地 Stdio MCP 服务器（子进程模式）。
     */
    public void addServer(String name, List<String> command, Map<String, String> env) {
        McpTransport transport = new StdioMcpTransport(name, command, env);
        clients.put(name, new McpClient(name, transport));
    }

    /**
     * 添加远程 HTTP/SSE MCP 服务器。
     *
     * @param name    服务器标识名称
     * @param baseUrl MCP Server 的 URL（如 http://remote-host:3000/mcp）
     * @param headers 自定义请求头（如 Authorization token）
     */
    public void addRemoteServer(String name, String baseUrl, Map<String, String> headers) {
        McpTransport transport = new HttpSseMcpTransport(name, baseUrl, headers);
        clients.put(name, new McpClient(name, transport));
    }
